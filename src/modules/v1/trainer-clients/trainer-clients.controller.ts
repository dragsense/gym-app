...
export class TrainerClientsController {
    ...
    @ApiResponse({ status: 200, description: 'Trainer-client assignment retrieved successfully' })
    @ApiResponse({ status: 404, description: 'Trainer-client assignment not found' })
    async findOne(
    ...
    @ApiResponse({ status: 200, description: 'Trainer clients retrieved successfully' })
    ...
    @ApiResponse({ status: 200, description: 'Client trainers retrieved successfully' })
    ...
    @ApiResponse({ status: 201, description: 'Trainer-client assignment created successfully' })
    ...
    async create(@Body() createDto: CreateTrainerClientDto) {
        const assignment = await this.trainerClientsService.createTrainerClient(createDto);
        return { message: 'Trainer-client assignment created successfully', data: assignment };
    }

    @Patch(':id')
    @ApiOperation({ summary: 'Update trainer-client assignment' })
    @ApiParam({ name: 'id', type: 'number' })
    @ApiResponse({ status: 200, description: 'Trainer-client assignment updated successfully' })
    @ApiResponse({ status: 404, description: 'Trainer-client assignment not found' })
    async update(
        @Param('id', ParseIntPipe) id: number,
        @Body() updateDto: UpdateTrainerClientDto
    ) {
        const assignment = await this.trainerClientsService.updateTrainerClient(id, updateDto);
        return { message: 'Trainer-client assignment updated successfully', data: assignment };
    }

    @Delete(':id')
    @ApiOperation({ summary: 'Delete trainer-client assignment' })
    @ApiParam({ name: 'id', type: 'number' })
    @ApiResponse({ status: 200, description: 'Trainer-client assignment deleted successfully' })
    @ApiResponse({ status: 404, description: 'Trainer-client assignment not found' })
