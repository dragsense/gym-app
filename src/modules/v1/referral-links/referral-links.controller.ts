...
export class ReferralLinksController {
  ...
  create(@Body() createReferralLinkDto: CreateReferralLinkDto,
    ...
    return this.referralLinksService.createReferralLink(createReferralLinkDto, userId);
  }

  @Get()
  @ApiOperation({ summary: 'Get all referral links with pagination and filtering' })
  ...
  findAll(@Query() query: ReferralLinkListDto, @AuthUser() user: any) {
    return this.referralLinksService.get({...query, createdByUserId: user.id}, ReferralLinkListDto);
  }
  ...
  @ApiResponse({ status: 404, description: 'Referral link not found' })
  update(@Param('id', ParseIntPipe) id: number, @Body() updateReferralLinkDto: UpdateReferralLinkDto) {
    return this.referralLinksService.updateReferralLink(id, updateReferralLinkDto);
  }
