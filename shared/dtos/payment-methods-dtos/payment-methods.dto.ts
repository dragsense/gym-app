...
import { EPaymentMethodType, EPaymentMethodStatus } from '../../enums/payment-methods.enum';
import { IPaymentMethod } from '../../interfaces/payment-methods.interface';

// Create Payment Method DTO
export class CreatePaymentMethodDto {
  @ApiProperty({ example: 'stripe', description: 'Payment method type', enum: EPaymentMethodType })
  @IsEnum(EPaymentMethodType)
  @IsNotEmpty()
  @FieldType("select", true)
  @FieldOptions(Object.values(EPaymentMethodType).map(v => ({ value: v, label: v.charAt(0).toUpperCase() + v.slice(1) })))
...
export class UpdatePaymentMethodDto extends PartialType(OmitType(CreatePaymentMethodDto, [])) { }

export class PaymentMethodListDto extends ListQueryDto<IPaymentMethod> {
  @ApiPropertyOptional({ example: 'stripe', description: 'Filter by payment method type', enum: EPaymentMethodType })
  @IsEnum(EPaymentMethodType)
  @IsOptional()
...
export class PaymentMethodPaginatedDto {
  @ApiProperty({ type: [PaymentMethodDto], description: 'List of payment methods' })
