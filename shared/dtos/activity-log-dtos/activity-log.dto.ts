...
export class ActivityLogDto {
    ...
    @ApiProperty({ example: 'User logged in successfully', description: 'Activity description' })
    description: string;
    ...
    @ApiPropertyOptional({ example: '/api/v1/users', description: 'API endpoint accessed' })
    endpoint?: string;
...
export class ActivityLogListDto extends ListQueryDto {
    ...
    @FieldOptions(Object.values(EActivityType).map(v => ({ value: v, label: v })))
    type?: EActivityType;
