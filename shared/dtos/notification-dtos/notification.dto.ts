...
import { ENotificationType, ENotificationPriority, ENotificationStatus } from '../../enums/notification.enum';
...
export class NotificationListDto extends ListQueryDto {
    ...
    @FieldOptions(Object.values(ENotificationType).map(v => ({ value: v, label: v })))
    type?: ENotificationType;
    ...
    @FieldType('select', false)
    @FieldOptions(Object.values(ENotificationPriority).map(v => ({ value: v, label: v })))
    priority?: ENotificationPriority;
