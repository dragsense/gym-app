...
export class UserAvailabilityService extends CrudService<UserAvailability> {
  ...
  async createOrUpdateUserAvailability(createUserAvailabilityDto: CreateUserAvailabilityDto, userId: number): Promise<UserAvailability> {
    const existingAvailability = await this.getSingle({
      userId,
    });

    if (existingAvailability) {
      return await this.update(existingAvailability.id, createUserAvailabilityDto);
    } else {
