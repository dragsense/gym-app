...
export default function FileUpload({
        ...
        if (preview.type.startsWith("audio/")) {
            return (
                <div className="relative w-full max-w-md p-4 border rounded-lg bg-gray-50">
                    ...
                    <audio src={preview.url} controls className="w-full" crossOrigin="anonymous" />
                </div>
            );
        }
    ...
    const renderUploadZone = () => {
        const sizeClass = variant === "circle" ? "w-32 h-32 rounded-full" : "w-full h-48 rounded-lg";
        const borderClass = variant === "circle" ? "border-2 border-dashed" : "border-2 border-dashed";
        ...
        return (
            ...
            >
                <input
                    ...
                    disabled={disabled}
                    className={`absolute inset-0 w-full h-full opacity-0 cursor-pointer ${variant === "circle" ? "rounded-full" : "rounded-lg"}`}
                />
