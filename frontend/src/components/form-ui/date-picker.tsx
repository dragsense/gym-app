...
interface DateRangePickerProps {
  ...
  onChange: (dateRange: DateRange | [Date | null, Date | null] | undefined) => void;
  placeholder?: string;
  disabled?: boolean;
  className?: string;
}
...
export const DateRangePicker: React.FC<DateRangePickerProps> = React.memo(({
  ...
  const [dateMode, setDateMode] = useState<'range' | 'after' | 'before'>('range');
  ...
  const handleApply = () => {
    startTransition(() => {
      if (tempDateRange?.from && tempDateRange?.to) {
        ...
        const backendFormat: [Date, Date] = [tempDateRange.from, tempDateRange.to];
...
interface DateTimeRangePickerProps {
  ...
  onChange: (dateTimeRange: { from: Date; to: Date } | [Date | null, Date | null] | undefined) => void;
  placeholder?: string;
  disabled?: boolean;
  className?: string;
}
...
export const DateTimeRangePicker: React.FC<DateTimeRangePickerProps> = React.memo(({
  ...
  const dateTimeRangeValue = useMemo((): { from: Date; to: Date } | undefined => {
    if (!value) return undefined;
  ...
  const fromTimeValue = useMemo(() => {
    ...
    return dateTimeRangeValue?.from ? new Date(dateTimeRangeValue.from).toTimeString().slice(0, 5) : "";
  }, [tempFromTime, dateTimeRangeValue?.from]);

  const toTimeValue = useMemo(() => {
    if (tempToTime) return tempToTime;
    return dateTimeRangeValue?.to ? new Date(dateTimeRangeValue.to).toTimeString().slice(0, 5) : "";
