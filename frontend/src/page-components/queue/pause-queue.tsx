...
import { pauseQueue } from '@/services/queue.api';
import { Dialog, DialogContent } from '@/components/ui/dialog';

interface IPauseQueueProps extends TListHandlerComponentProps<TListHandlerStore<IQueue, any, any>> { }
...
export default function PauseQueue({
  ...
  const handlePause = useCallback(async () => {
    startTransition(async () => {
      try {
        await pauseQueue(deferredPayload);
  ...
  return (
    <Dialog data-component-id={componentId} {...dialogProps}>
      <DialogContent>
        <AppDialog
          title="Pause Queue"
          description={`Are you sure you want to pause the queue "${deferredPayload}"? This will stop processing new jobs.`}
        >
