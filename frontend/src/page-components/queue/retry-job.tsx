...
export default function RetryJob({
  ...
  const handleRetry = async () => {
    try {
      setIsLoading(true);
      await retryJob(payload.queueName, payload.jobId);
  ...
  return (
    <Dialog 
      ...
      <DialogContent>
        <AppDialog
          title="Retry Job"
          description={`Are you sure you want to retry job "${payload?.jobId}"? This will restart the job from the beginning.`}
        >
